...
class Conjunction(STLNode):
    """Phi_0 AND Phi_1 AND ... AND Phi_n  -->  rho = min over all children's rho

    Children are supplied as arg0, arg1, ... (at least two required).
    """

    def __init__(self, *children: STLNode) -> None:
        if len(children) < 2:
            raise ValueError("Conjunction requires at least 2 children (arg0, arg1, ...).")
        self.children = list(children)

    def robustness(self, t: np.ndarray, x: np.ndarray) -> np.ndarray:
        return np.minimum.reduce([c.robustness(t, x) for c in self.children])

    def horizon(self) -> float:
        # ||phi1 AND phi2 AND ...|| = max(||phi_i||)
        return max(c.horizon() for c in self.children)
...
class Disjunction(STLNode):
    """Phi_0 OR Phi_1 OR ... OR Phi_n  -->  rho = max over all children's rho
    ...
    def __init__(self, *children: STLNode) -> None:
        if len(children) < 2:
            raise ValueError("Disjunction requires at least 2 children (arg0, arg1, ...).")
        self.children = list(children)

    def robustness(self, t: np.ndarray, x: np.ndarray) -> np.ndarray:
        return np.maximum.reduce([c.robustness(t, x) for c in self.children])

    def horizon(self) -> float:
        # ||phi1 OR phi2 OR ...|| = max(||phi_i||)
        return max(c.horizon() for c in self.children)
...
# --------------------
# Smooth aggregation helpers
# --------------------

def smooth_min(values: np.ndarray, beta: float) -> float:
    return float(-1.0 / beta * np.log(np.sum(np.exp(-beta * values))))
...
class TemporalOperator(STLNode):
    ...
    Parameters
    ----------
    smooth : bool   If True, use differentiable smooth_min/smooth_max instead
                    of hard min/max.  Default False.
    beta   : float  Smoothness parameter (larger = closer to hard min/max).
                    Only used when smooth=True.  Default 10.0.
    """

    def __init__(self, a: float, b: float, child: STLNode,
                 smooth: bool = False, beta: float = 10.0) -> None:
        if a < 0 or b < a:
    ...
    @abstractmethod
    def hard_aggregate(self, values: np.ndarray) -> float:
        """Hard min (G) or max (F) aggregation."""

    @abstractmethod
    def smooth_aggregate(self, values: np.ndarray) -> float:
        """Smooth differentiable min (G) or max (F) aggregation."""
    ...
    def robustness(self, t: np.ndarray, x: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(t)):
            ...
            if len(idx) == 0:
                ...
                #apply hard or smooth min (G) / max (F) over interval [a,b]
                if self.smooth:
                    rho[i] = self.smooth_aggregate(child_rho[idx])
                else:
                    rho[i] = self.hard_aggregate(child_rho[idx])
        return rho
...
class Globally(TemporalOperator):
    ...
    def smooth_aggregate(self, values: np.ndarray) -> float:
        return smooth_min(values, self.beta)
...
class Eventually(TemporalOperator):
    ...
    def smooth_aggregate(self, values: np.ndarray) -> float:
        return smooth_max(values, self.beta)
...
def _parse_node(cfg: dict, pnf: bool = False) -> STLNode:
    ...
    if node_type == "NOT":
        ...
        # De Morgan: NOT (φ AND ψ)  →  (NOT φ_i) OR ... for all children
        if child_type == "AND":
            neg_children = [
                _parse_node({"type": "NOT", "child": child_cfg[f"arg{i}"]}, pnf=True)
                for i in range(10) if f"arg{i}" in child_cfg
            ]
            return Disjunction(*neg_children)

        # De Morgan: NOT (φ OR ψ)  →  (NOT φ_i) AND ... for all children
        if child_type == "OR":
            neg_children = [
                _parse_node({"type": "NOT", "child": child_cfg[f"arg{i}"]}, pnf=True)
                for i in range(10) if f"arg{i}" in child_cfg
            ]
            return Conjunction(*neg_children)
    ...
    if node_type == "AND":
        return Conjunction(*_parse_variadic_children(cfg, pnf=pnf))

    if node_type == "OR":
        return Disjunction(*_parse_variadic_children(cfg, pnf=pnf))

    if node_type == "G":
        a, b   = require_interval(cfg)
        smooth = bool(cfg.get("smooth", False))
        beta   = float(cfg.get("beta", 10.0))
        return Globally(a, b, require_child(cfg, "child", pnf=pnf), smooth=smooth, beta=beta)

    if node_type == "F":
        a, b   = require_interval(cfg)
        smooth = bool(cfg.get("smooth", False))
        beta   = float(cfg.get("beta", 10.0))
        return Eventually(a, b, require_child(cfg, "child", pnf=pnf), smooth=smooth, beta=beta)
...
def _parse_variadic_children(cfg: dict, pnf: bool = False) -> list[STLNode]:
    """
    Collect arg0, arg1, ..., arg9 from cfg and parse each as an STLNode.
    At least two children (arg0 and arg1) are required.
    """
    children = []
    for i in range(10):
        key = f"arg{i}"
        if key in cfg:
            children.append(_parse_node(cfg[key], pnf=pnf))
        elif i >= 2:
            # arg0 and arg1 are mandatory; stop scanning after the first gap
            break
        else:
            raise ValueError(
                f"Node of type '{cfg.get('type')}' requires at least 'arg0' and 'arg1'."
            )
    return children
